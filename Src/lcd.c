...
/******************************************************************************
      ...
      返回值：  无
******************************************************************************/
void LCD_Fill(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend, lv_color_t *color_p)
{
...
#else
    HAL_SPI_Transmit(&LCD_SPI, (uint8_t *)&color_p->full, (xend - xsta + 1) * (yend - ysta + 1), 10); // 启动SPI传输
#endif
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef LCD_SPI_Transmit(SPI_HandleTypeDef *hspi, lv_color_t *pData, uint32_t Size)
{
    uint32_t tickstart;
